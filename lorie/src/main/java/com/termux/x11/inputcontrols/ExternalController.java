...
public class ExternalController {
    ...
    private void processTriggerButton(MotionEvent event) {
        state.triggerL = MathUtils.clamp(Math.max(event.getAxisValue(MotionEvent.AXIS_LTRIGGER), event.getAxisValue(MotionEvent.AXIS_BRAKE)) - EPSILON, 0.0f, 1.0f);
        state.triggerR = MathUtils.clamp(Math.max(event.getAxisValue(MotionEvent.AXIS_RTRIGGER), event.getAxisValue(MotionEvent.AXIS_GAS)) - EPSILON, 0.0f, 1.0f);
    }
    ...
    public static void updateConnectedControllers(ArrayList<ExternalController> connectedControllers) {
        ...
        for (int i = connectedControllers.size()-1; i >= 0; i--) {
            ...
            for (int deviceId : deviceIds) if (deviceId == controller.getDeviceId()) { connected = true; break; }
